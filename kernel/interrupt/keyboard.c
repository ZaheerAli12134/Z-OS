...
void keyboardHandler(struct InterruptRegisters* regs){
    char charCode = inPortB(0x60) & 0x7F; //waht key is pressed
    char pressed = inPortB(0x60) & 0x80; //if its pressed or held down
    ...
    switch(charCode){
        ...
        case 42: //shift key
            ...
            break;
        case 58:
            if(pressed == 0 && !capsLock){
                capsLock = true;
            }
            else if(pressed == 0 && capsLock){
                capsLock = false;
        ...
        default:
            if(pressed == 0){
                if(capsLock || shiftLock){
                    printChar(uppercase[charCode]);
                }
                else{
                    printChar(lowercase[charCode]);
                }
            }
